Feature: Validate changes from usage of frameworks Quarkus MongoDB skill

Background:
  Given the skill "415-frameworks-quarkus-mongodb"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Quarkus MongoDB guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Quarkus MongoDB guidance"
  And the local generated skill path ".agents/skills/415-frameworks-quarkus-mongodb"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/415-frameworks-quarkus-mongodb" is applied to the request
