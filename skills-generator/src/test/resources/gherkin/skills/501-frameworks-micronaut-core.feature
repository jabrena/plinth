Feature: Validate changes from usage of frameworks Micronaut core skill

Background:
  Given the skill "501-frameworks-micronaut-core"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Micronaut core guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Micronaut core guidance"
  And the local generated skill path ".agents/skills/501-frameworks-micronaut-core"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/501-frameworks-micronaut-core" is applied to the request
