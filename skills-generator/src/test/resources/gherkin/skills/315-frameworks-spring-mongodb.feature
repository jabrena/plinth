Feature: Validate changes from usage of frameworks Spring MongoDB skill

Background:
  Given the skill "315-frameworks-spring-mongodb"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Spring MongoDB guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Spring MongoDB guidance"
  And the local generated skill path ".agents/skills/315-frameworks-spring-mongodb"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/315-frameworks-spring-mongodb" is applied to the request
