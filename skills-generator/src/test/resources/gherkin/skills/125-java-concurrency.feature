Feature: Validate changes from usage of java concurrency skill

Background:
  Given the skill "125-java-concurrency"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply java concurrency guidance from a concrete Java request
  Given the user request is "Review a Java project and apply java concurrency guidance"
  And the local generated skill path ".agents/skills/125-java-concurrency"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/125-java-concurrency" is applied to the request
