Feature: Validate changes from usage of frameworks Quarkus validation skill

Background:
  Given the skill "403-frameworks-quarkus-validation"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Quarkus validation guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Quarkus validation guidance"
  And the local generated skill path ".agents/skills/403-frameworks-quarkus-validation"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/403-frameworks-quarkus-validation" is applied to the request
