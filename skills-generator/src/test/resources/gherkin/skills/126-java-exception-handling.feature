Feature: Validate changes from usage of java exception handling skill

Background:
  Given the skill "126-java-exception-handling"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply java exception handling guidance from a concrete Java request
  Given the user request is "Review a Java project and apply java exception handling guidance"
  And the local generated skill path ".agents/skills/126-java-exception-handling"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/126-java-exception-handling" is applied to the request
