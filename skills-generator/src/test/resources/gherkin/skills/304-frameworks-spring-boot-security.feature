Feature: Validate changes from usage of frameworks Spring Boot security skill

Background:
  Given the skill "304-frameworks-spring-boot-security"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Spring Boot security guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Spring Boot security guidance"
  And the local generated skill path ".agents/skills/304-frameworks-spring-boot-security"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/304-frameworks-spring-boot-security" is applied to the request
