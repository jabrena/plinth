Feature: Validate changes from usage of agile feature skill

Background:
  Given the skill "013-agile-feature"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply agile feature guidance from a concrete Java request
  Given the user request is "Review a Java project and apply agile feature guidance"
  And the local generated skill path ".agents/skills/013-agile-feature"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/013-agile-feature" is applied to the request
