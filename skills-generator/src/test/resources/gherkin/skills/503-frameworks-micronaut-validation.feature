Feature: Validate changes from usage of frameworks Micronaut validation skill

Background:
  Given the skill "503-frameworks-micronaut-validation"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Micronaut validation guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Micronaut validation guidance"
  And the local generated skill path ".agents/skills/503-frameworks-micronaut-validation"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/503-frameworks-micronaut-validation" is applied to the request
