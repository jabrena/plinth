Feature: Validate changes from usage of planning Jira skill

Background:
  Given the skill "044-planning-jira"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply planning Jira guidance from a concrete Java request
  Given the user request is "Review a Java project and apply planning Jira guidance"
  And the local generated skill path ".agents/skills/044-planning-jira"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/044-planning-jira" is applied to the request
