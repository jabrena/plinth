Feature: Validate changes from usage of java functional programming skill

Background:
  Given the skill "142-java-functional-programming"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply java functional programming guidance from a concrete Java request
  Given the user request is "Review a Java project and apply java functional programming guidance"
  And the local generated skill path ".agents/skills/142-java-functional-programming"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/142-java-functional-programming" is applied to the request
