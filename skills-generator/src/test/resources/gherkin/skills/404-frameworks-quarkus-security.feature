Feature: Validate changes from usage of frameworks Quarkus security skill

Background:
  Given the skill "404-frameworks-quarkus-security"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Quarkus security guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Quarkus security guidance"
  And the local generated skill path ".agents/skills/404-frameworks-quarkus-security"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/404-frameworks-quarkus-security" is applied to the request
