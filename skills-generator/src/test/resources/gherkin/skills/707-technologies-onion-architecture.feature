Feature: Validate changes from usage of Java Onion architecture skill

Background:
  Given the skill "707-technologies-onion-architecture"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Review Java project boundaries for Onion architecture dependency direction
  Given a Java project with domain, application, adapter, and infrastructure packages
  And the user request is "Review this Java project for Onion architecture boundary violations and suggest optional architecture tests"
  And the local generated skill path ".agents/skills/707-technologies-onion-architecture"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/707-technologies-onion-architecture" is applied to the Java project
  Then the skill reads "references/707-technologies-onion-architecture.md"
  And the skill maps domain models, domain services, application or use-case services, ports, adapters, and infrastructure concerns using code evidence
  And the skill verifies inward dependency direction with packages, classes, imports, module dependencies, build files, or tests inspected
