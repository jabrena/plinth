Feature: Validate changes from usage of agents MD skill

Background:
  Given the skill "200-agents-md"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply agents MD guidance from a concrete Java request
  Given the user request is "Review a Java project and apply agents MD guidance"
  And the local generated skill path ".agents/skills/200-agents-md"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/200-agents-md" is applied to the request
