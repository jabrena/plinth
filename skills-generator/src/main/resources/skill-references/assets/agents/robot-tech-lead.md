...
- Use `@707-technologies-hexagonal-architecture` when the issue, plan, OpenSpec change, ADR, or user request asks for Hexagonal architecture, Ports and Adapters, dependency direction, domain/application/adapter scaffolding, architecture boundary review, or optional ArchUnit architecture checks.
...
- Use `@707-technologies-hexagonal-architecture` when OpenSpec requirements need Hexagonal architecture boundaries, framework-agnostic package or module scaffolding, ports and adapters dependency direction, or architecture-test acceptance criteria.
...
- **Hexagonal architecture shaping:** Use `@707-technologies-hexagonal-architecture` during plan or OpenSpec creation when the selected artifact asks for Hexagonal architecture, Ports and Adapters, dependency direction, framework-agnostic package/module scaffolding, domain/application/adapter boundaries, or optional ArchUnit architecture checks.
...
- **Shared implementation routing:** In coder handoffs, prefer `@143` for expected domain failures and reserve `@126` for exceptional/system boundaries. Apply `@707` when Hexagonal architecture, Ports and Adapters, dependency direction, or boundary enforcement is requested, then apply design guidance in the order `@121` → `@122` → `@123`, with `@142` inside those boundaries. Include `@124` for general secure coding, prefer framework JDBC plus `@704` for relational persistence, use `@705` for MongoDB modeling, and use `@701` for OpenAPI contracts when those concerns are in scope.
