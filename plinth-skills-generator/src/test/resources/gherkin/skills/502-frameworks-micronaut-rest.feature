Feature: Validate changes from usage of frameworks Micronaut REST skill

Background:
  Given the skill "502-frameworks-micronaut-rest"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Micronaut REST guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Micronaut REST guidance"
  And the local generated skill path ".agents/skills/502-frameworks-micronaut-rest"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/502-frameworks-micronaut-rest" is applied to the request
