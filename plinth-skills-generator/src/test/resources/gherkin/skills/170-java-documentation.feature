Feature: Validate changes from usage of java documentation skill

Background:
  Given the skill "170-java-documentation"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply java documentation guidance from a concrete Java request
  Given the user request is "Review a Java project and apply java documentation guidance"
  And the local generated skill path ".agents/skills/170-java-documentation"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/170-java-documentation" is applied to the request
