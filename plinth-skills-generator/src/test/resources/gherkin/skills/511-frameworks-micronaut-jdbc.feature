Feature: Validate changes from usage of frameworks Micronaut JDBC skill

Background:
  Given the skill "511-frameworks-micronaut-jdbc"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Micronaut JDBC guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Micronaut JDBC guidance"
  And the local generated skill path ".agents/skills/511-frameworks-micronaut-jdbc"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/511-frameworks-micronaut-jdbc" is applied to the request
