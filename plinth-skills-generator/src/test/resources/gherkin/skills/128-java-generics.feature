Feature: Validate changes from usage of java generics skill

Background:
  Given the skill "128-java-generics"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply java generics guidance from a concrete Java request
  Given the user request is "Review a Java project and apply java generics guidance"
  And the local generated skill path ".agents/skills/128-java-generics"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/128-java-generics" is applied to the request
