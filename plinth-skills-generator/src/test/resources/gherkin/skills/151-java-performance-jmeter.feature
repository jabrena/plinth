Feature: Validate changes from usage of java performance JMeter skill

Background:
  Given the skill "151-java-performance-jmeter"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply java performance JMeter guidance from a concrete Java request
  Given the user request is "Review a Java project and apply java performance JMeter guidance"
  And the local generated skill path ".agents/skills/151-java-performance-jmeter"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/151-java-performance-jmeter" is applied to the request
