Feature: Validate changes from usage of java profiling analyze skill

Background:
  Given the skill "162-java-profiling-analyze"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply java profiling analyze guidance from a concrete Java request
  Given the user request is "Review a Java project and apply java profiling analyze guidance"
  And the local generated skill path ".agents/skills/162-java-profiling-analyze"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/162-java-profiling-analyze" is applied to the request
