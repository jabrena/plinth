Feature: Validate changes from usage of frameworks Quarkus Kafka skill

Background:
  Given the skill "414-frameworks-quarkus-kafka"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Quarkus Kafka guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Quarkus Kafka guidance"
  And the local generated skill path ".agents/skills/414-frameworks-quarkus-kafka"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/414-frameworks-quarkus-kafka" is applied to the request
