Feature: Validate changes from usage of frameworks Spring Kafka skill

Background:
  Given the skill "314-frameworks-spring-kafka"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Spring Kafka guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Spring Kafka guidance"
  And the local generated skill path ".agents/skills/314-frameworks-spring-kafka"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/314-frameworks-spring-kafka" is applied to the request
