Feature: Validate changes from usage of java type design skill

Background:
  Given the skill "122-java-type-design"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply java type design guidance from a concrete Java request
  Given the user request is "Review a Java project and apply java type design guidance"
  And the local generated skill path ".agents/skills/122-java-type-design"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/122-java-type-design" is applied to the request
