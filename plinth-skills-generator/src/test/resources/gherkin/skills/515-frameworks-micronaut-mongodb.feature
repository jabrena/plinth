Feature: Validate changes from usage of frameworks Micronaut MongoDB skill

Background:
  Given the skill "515-frameworks-micronaut-mongodb"
  And the folder "examples" has no git changes

@acceptance-test
Scenario: Apply frameworks Micronaut MongoDB guidance from a concrete Java request
  Given the user request is "Review a Java project and apply frameworks Micronaut MongoDB guidance"
  And the local generated skill path ".agents/skills/515-frameworks-micronaut-mongodb"
  And the folder "examples" has no git changes
  When the skill ".agents/skills/515-frameworks-micronaut-mongodb" is applied to the request
