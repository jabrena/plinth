...
The project provides a collection of Cursor rules for Java (System prompts) that help software engineers in their daily work. The available Cursor Rules for Java cover aspects like Build system based on Maven, Design, Coding, Testing, Refactoring & JMH Benchmarking, Performance testing (JMeter), Profiling (Async profiler, jps, jstack, jcmd & jstat) & Documentation.
...
If you interact with Claude, you ask the following question:

![](./images/claude-java-hardest-parts.png)

```bash
What are the hardest parts in Java for a software engineer?
```
