# hexagonal-architecture-technology-skill-reference Specification

## Purpose
Define the requirements for the framework-agnostic Hexagonal architecture technology skill, including ports and adapters boundary review, dependency direction, optional architecture tests, and source/generated-output ownership.
## Requirements
### Requirement: Hexagonal architecture technology skill exists

The repository MUST add a generated `707-technologies-hexagonal-architecture` skill for framework-agnostic Hexagonal architecture guidance in Java projects.
...
### Requirement: Hexagonal architecture guidance covers boundary review

The `707-technologies-hexagonal-architecture` skill MUST help users review Java application boundaries according to Hexagonal architecture dependency direction and responsibility placement.

#### Scenario: Guidance covers core Hexagonal architecture concerns

- **WHEN** generated `references/707-technologies-hexagonal-architecture.md` is inspected
- **THEN** it explains domain independence from infrastructure and framework APIs
- **AND** it explains application/use-case orchestration through inbound and outbound ports
- **AND** it explains driving adapters calling inbound ports and driven adapters implementing outbound ports
- **AND** it explains package or module dependency direction checks between the core and adapters
...
- **AND** recommendations preserve the direction that the application core does not depend on adapters

### Requirement: ArchUnit verification is supported without being mandatory

The `707-technologies-hexagonal-architecture` skill MUST include ArchUnit-aware verification guidance while avoiding mandatory dependency changes for every project.

#### Scenario: ArchUnit guidance is present

- **WHEN** generated `references/707-technologies-hexagonal-architecture.md` is inspected
- **THEN** it references ArchUnit-style architecture verification for Hexagonal architecture boundaries
...
- **AND** it does not claim ArchUnit must be added to every project before Hexagonal architecture guidance can be useful

### Requirement: Hexagonal architecture skill stays framework-agnostic

The `707-technologies-hexagonal-architecture` skill MUST stay focused on architecture boundaries and avoid framework runtime implementation guidance unless explicitly requested.
...
- **WHEN** `707-technologies-hexagonal-architecture` is applied
- **THEN** the skill identifies that the concern is outside pure Hexagonal architecture boundary review
...
The repository MUST add acceptance coverage for `707-technologies-hexagonal-architecture`.
...
- **AND** local `.agents/skills/707-technologies-hexagonal-architecture` output is refreshed only by running the documented generator command
