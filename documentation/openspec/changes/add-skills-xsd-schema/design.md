...
This `/explore-design` pass engaged four genuine open questions and confirmed one already-settled decision holds. Findings are grounded in direct evidence gathered during this review (the live PML 0.8.0 `pml.xsd`, the shipped `agents.xsd`/`commands.xsd` precedent, and the actual `skill-indexes/*.xml` content), not assumption:

1. **Relative schema-path resolution** (`xsi:noNamespaceSchemaLocation` value in migrated `skill-indexes/*.xml`): firmed up to `../skills.xsd`, matching the already-shipped `../agents.xsd` / `../commands.xsd` convention exactly (same directory topology: artifact subdirectory one level below the resources root holding the schema). Confirmed, by reading `RemoteSchemaValidationTest` and `CommandSchemaTest`, that none of the four validation layers actually resolve the schema via this attribute — all four load `skills.xsd` by a fixed classpath/file path. The attribute is IDE/developer-tooling convenience and self-documentation only. This is no longer an open question requiring implementation-time confirmation of the *value*; only the byte-for-byte migration mechanics remain implementation work.
2. **Fail-fast vs. collect-all-errors for the new `SkillsGenerator` runtime validation step**: recommend fail-fast, per-file, matching the existing uniform behavior of every other failure mode in `SkillsGenerator` (`generateAllSkills()` is a `Stream.map()` pipeline — any thrown exception already aborts the whole run today). Comprehensive "every file's status" visibility is already delivered by the separate parameterized Maven test layer, not owed by the runtime layer.
3. **`SkillIndexSchemaValidationTest` structure**: confirmed correct as a separate class (not a `RemoteSchemaValidationTest` extension), with `plinth-commands-generator`'s already-shipped `CommandSchemaTest` as a concrete, provably-analogous structural template (enumerate via the artifact's own Indexes class, load schema once via classpath resource, cover complete inventory plus targeted invalid fixtures).
4. **ADR-008 task sequencing** (`tasks.md` section 9 after section 8): confirmed correct, not a defect. ADR-008 must describe a *completed, verified* scope, so authoring it after integrated validation (section 8: `mvnw verify` + `openspec validate --all`) passes is the right order. One genuine gap found and fixed: no task validated the new Markdown files (`ADR-008-*.md`, updated `documentation/adr/README.md`) with this repository's own required Markdown-validation step for Markdown-only changes — added as task 9.4.
5. **Schema-validity risk for the 125 skill-index files** (Open Question 1): investigated concretely rather than left as a placeholder. The three files using the `xi:include`-inside-CDATA splice pattern (`033-skill.xml`, `111-skill.xml`, `112-skill.xml`) are confirmed safe against the actual PML 0.8.0 `pml.xsd` — `goal` and `step-content` are declared `mixed="true"` with a lax `xs:any` wildcard specifically permissive of arbitrary embedded elements, independent of XInclude expansion order. A separate, more material finding: in the actual fetched PML 0.8.0 schema, `goal` is the *only* required child element of `<prompt>` — `metadata/description` and `title` (the fields the current `examples/xml/invalid-skill-index-example.xml` plan and the `pml-skills-schema` spec scenario describe omitting) are both `minOccurs="0"` and would NOT fail validation if omitted. This is corrected below and in the spec file, since the currently-planned invalid fixture would not actually demonstrate a validation failure.
...
Migrated documents reference the local copy with a relative `xsi:noNamespaceSchemaLocation="../skills.xsd"`. This value is a firm recommendation, not an implementation-time open item: `skill-indexes/` sits exactly one directory below `plinth-skills-generator/src/main/resources/`, where `skills.xsd` will live — the identical topology already used by `plinth-agents-generator/src/main/resources/agents/robot-*.xml` (`xsi:noNamespaceSchemaLocation="../agents.xsd"`) and `plinth-commands-generator/src/main/resources/commands/*.xml` (`xsi:noNamespaceSchemaLocation="../commands.xsd"`). Both sibling schemas already ship this exact convention, so `../skills.xsd` is not a guess but a directly transferable, proven value.

**Functional independence of this attribute (confirmed by reading the code, not assumed):** none of the four validation layers actually resolve `skills.xsd` via this attribute. `RemoteSchemaValidationTest` loads its schema from a hardcoded `REMOTE_XSD` constant URL; `plinth-commands-generator`'s analogous `CommandSchemaTest` loads `commands.xsd` via `getClassLoader().getResource("commands.xsd")` — both independent of whatever `xsi:noNamespaceSchemaLocation` says inside any given source file. The planned `SkillIndexSchemaValidationTest` and the `SkillsGenerator` runtime step are expected to follow the same pattern: load `skills.xsd` once via a fixed classpath resource name, not by reading the attribute out of each parsed document. Consequently, the attribute's only functional role is IDE-level developer experience (inline validation-as-you-type) and human-readable self-documentation of the migrated file's schema; it does not need to be "confirmed during implementation" beyond the mechanical find-and-replace across all 125 files, which `xmllint --noout --schema` (task 2.4) and the new Maven test (task 4.1) already verify structurally regardless of the attribute's value.
...
**Confirmed with a concrete, already-shipped template:** `plinth-commands-generator/src/test/java/info/jab/pml/CommandSchemaTest.java` is the directly analogous precedent for `commands.xsd` — a schema-specific test class, distinct from `RemoteSchemaValidationTest`, that (a) enumerates the complete inventory via the artifact's own Indexes class (`CommandIndexes.commandSources()`), (b) loads the schema once via `getClassLoader().getResource("commands.xsd")`, (c) covers targeted invalid fixtures built as inline XML strings alongside the full-inventory pass, and (d) asserts on the schema's own declared shape. `SkillIndexSchemaValidationTest` should follow this same shape, enumerating via `SkillIndexes.skillDescriptors()` filtered to `useXml == true`. This resolves the "is separate-test reasoning thin?" check raised during design review: it is not thin — it is a proven, already-shipped pattern in a sibling generator module.
...
- **Making "no network access" an observable, enforced property (found during this review, closes a gap):** `RemoteSchemaValidationTest` deliberately sets `schemaFactory.setProperty(XMLConstants.ACCESS_EXTERNAL_DTD, "all")` / `ACCESS_EXTERNAL_SCHEMA, "all"` because it needs network access to fetch the remote schema. `plinth-commands-generator`'s `CommandSchemaTest`, by contrast, sets both to `""` (deny all external resource access) when building its `SchemaFactory` for the local `commands.xsd`. The new `SkillIndexSchemaValidationTest` and the `SkillsGenerator` runtime validation step should follow `CommandSchemaTest`'s `""` setting, not `RemoteSchemaValidationTest`'s `"all"` setting. This turns "validation does not require network access" from an implicit design intent into an enforced, observable property: if any migrated skill-index document or `skills.xsd` itself ever referenced an external DTD or schema, JAXP would fail loudly under `""` rather than silently succeeding via a hidden network call. This closes the one concrete gap this review found in the offline-validation acceptance scenario, where no task previously specified how "no network access" would be verified rather than merely designed-for.
- **Fail-fast, not collect-all-errors (recommended, evidence-based):** `SkillsGenerator.generateAllSkills()` is `SkillIndexes.skillDescriptors().map(d -> generateSkill(...))` — a `Stream.map()` pipeline. Every existing failure mode in this class (missing skill resource, missing XSLT, transform failure) already throws a `RuntimeException` that aborts the entire stream immediately, naming the one offending resource. The new schema-validation step should follow this same, already-uniform pattern: validate the one file being processed, throw immediately with the file name and violated constraint on failure, and let it abort the run — do not attempt to accumulate violations across all 125 files inside the generator. That "all files, all statuses" view is already delivered for free by the separate parameterized Maven test layer (`SkillIndexSchemaValidationTest`), where JUnit reports each file as its own pass/fail result. Adding collect-all behavior to the runtime layer would be a novel, inconsistent behavior relative to the rest of `SkillsGenerator` for no added benefit.
- **XInclude resolution and validation ordering (resolved, not deferred):** recommend validating the same XInclude-*expanded* `DOMSource` that `createXIncludeDomSource` already builds (i.e., add the `Validator.validate(...)` call against that DOM before it is handed to `Transformer`), rather than a second, separate raw-text parse pass. This reuses the existing parse, adds no new I/O, and is confirmed safe for the known highest-risk content: three files (`033-skill.xml`, `111-skill.xml`, `112-skill.xml`) embed `<xi:include>` directly inside `<step-content>` CDATA-adjacent text (splicing in `skill-references/assets/questions/*.xml` content). Both `goal` and `step-content` are declared in the actual PML 0.8.0 `pml.xsd` as `<xs:complexType mixed="true"><xs:sequence><xs:any namespace="##any" processContents="lax" minOccurs="0" maxOccurs="unbounded"/></xs:sequence></xs:complexType>` — a deliberately lax wildcard that tolerates arbitrary embedded elements (the raw `xi:include` element pre-expansion, or the expanded `<question-flow>` content post-expansion) equally. Validation order therefore does not change the outcome for these three files specifically; implementation should still run task 2.4's full-inventory `xmllint` pass first to catch any other file with a genuine structural problem (see Open Questions below for what "genuine" looks like given how permissive this schema actually is).
...
3. **Recommended addition found during this design review (not a separate ADR):** the colocated-schema, relative-path resolution convention — schema file colocated at the module's `src/main/resources/` root, artifact XML sources one directory below referencing it via `xsi:noNamespaceSchemaLocation="../<schema>.xsd"` — is already independently followed by `agents.xsd` and `commands.xsd` and would become a third instance with `skills.xsd`. This is small and mechanical enough to belong inside ADR-008's policy statement rather than warrant its own ADR; ADR-008 authoring should name it explicitly as part of "the policy" so future artifact schemas (if any) inherit it as an established convention instead of re-deriving it.
...
- `examples/xml/invalid-skill-index-example.xml` — a representative invalid skill-index document, omitting the `<goal>` element, intended to fail validation.

**Correction from the original plan (evidence-based):** the original plan described the invalid example as omitting `metadata/description` and `title`. Fetching and inspecting the actual PML 0.8.0 `pml.xsd` (`https://jabrena.github.io/pml/schemas/0.8.0/pml.xsd`, checked 2026-07-23 for this design review) shows that within `<prompt>`'s content model, `<goal>` is declared with default `minOccurs` (i.e., required) and is the *only* required child element — every other child, including `metadata` itself and, within `metadata`, `description` and `title`, is `minOccurs="0"` (optional). An example omitting only `metadata/description` and `title` would therefore validate successfully, not fail, and would not demonstrate the acceptance-criteria scenario it is meant to illustrate. The corrected invalid example omits `<goal>` instead — the one element PML 0.8.0 actually requires — which is expected to produce a genuine, meaningful `cvc-complex-type` (missing required element) diagnostic. This correction is also applied to the `pml-skills-schema` spec scenario below.

These illustrate the acceptance-criteria intent ("Given a representative valid/invalid skill definition, when it is validated against the proposed schema, then validation succeeds/fails with a meaningful validation error"). Because the exact cardinalities come from the copied PML 0.8.0 `pml.xsd` (not authored in this change), implementation MUST re-run `xmllint --noout --schema skills.xsd` against both examples once the real `skills.xsd` is in place and record the actual diagnostic text for the invalid case — the illustrative examples here are a design aid, not a substitute for that confirmation. Implementation should also reconfirm this reading at copy time, since the schema is externally owned and this review reflects its content as of 2026-07-23, not a version pinned by this OpenSpec change.
...
- [Risk] Some of the 125 files may not currently be fully compliant with PML 0.8.0 even though they reference it remotely (remote validation was never automated for `skill-indexes/`). → Mitigate by running the full-inventory validation early in implementation, before wiring runtime enforcement, and treat any failures as pre-existing defects to fix as part of this atomic migration. **This design review narrows where such failures are actually likely to originate:** the schema is very permissive (only `<goal>` is required at the `<prompt>` level; nearly every other element is `minOccurs="0"`), so realistic failure modes are element mis-*ordering* against the schema's strict `xs:sequence` (e.g. a file placing `<constraints>` before `<goal>`) or use of an undeclared element name — not missing-optional-field cases. The three files using the `xi:include`-in-CDATA pattern (`033-skill.xml`, `111-skill.xml`, `112-skill.xml`) were checked directly against the actual PML 0.8.0 schema and are safe (see "Generator runtime validation placement" above); they are not a special risk case after all.
...
- [Risk, low, newly identified] If the JAXP `Validator` implementation used for runtime validation performs schema-driven infoset augmentation (e.g. inserting declared default attribute values such as `toc/@auto-generate="true"`) on the same mutable DOM later fed to `Transformer`, output could theoretically shift for any file relying on an unspecified defaulted attribute. No skill-index file currently uses `<examples>/<toc>` (the only elements in PML 0.8.0 with declared `default=` values), so this is not expected to manifest, and the existing byte-for-byte comparison task (section 5) is the concrete safety net regardless — treat any unexpected diff there as a signal to validate against a `Source` that does not share DOM identity with the transform input.
- [Risk — resolved during this review] Relative `xsi:noNamespaceSchemaLocation` paths must resolve consistently for `xmllint`, the Java classpath, Maven tests, and CI. → Resolved: none of the four validation layers resolve `skills.xsd` via this attribute (each loads the schema by a fixed path/classpath name); the value `../skills.xsd` is a direct match for the already-shipped `agents.xsd`/`commands.xsd` convention. No further implementation-time confirmation of the *value* is needed, only mechanical migration.
...
|---|---|---|
| ADR-008: one XML Schema per generated artifact (skill-index, skill-reference, command, agent) | Author as part of this change's implementation, per the acceptance criteria confirmed on issue #991 (posted 2026-07-23); include the colocated-schema, `../<schema>.xsd` relative-path convention as part of "the policy" (already independently followed by `agents.xsd` and `commands.xsd`) | **Gating** (tracked in `tasks.md` section 9) |
...
| No new ADR beyond ADR-008 was found to be warranted by this review | The relative-path convention is small and mechanical enough to belong inside ADR-008's policy statement, not a separate ADR-009 | **Resolved** (this review) |
...
| Relative schema path in migrated `skill-indexes/*.xml`? | `xsi:noNamespaceSchemaLocation="../skills.xsd"`, matching the shipped `agents.xsd`/`commands.xsd` convention; confirmed functionally independent of all four validation layers | **Resolved** (this review) |
| Fail-fast vs. collect-all-errors for the new `SkillsGenerator` runtime validation step? | Fail-fast per file, matching every existing failure mode in `SkillsGenerator`; full-inventory visibility comes from the separate Maven test layer instead | **Resolved** (this review) |
| Schema-validation ordering relative to XInclude expansion? | Validate the already-built, XInclude-expanded `DOMSource` (no second parse pass); confirmed safe for the three known `xi:include`-in-CDATA files against the actual PML 0.8.0 wildcard content models | **Resolved** (this review) |
| Is the `SkillIndexSchemaValidationTest` "separate test" reasoning solid? | Yes — `CommandSchemaTest` in `plinth-commands-generator` is a directly analogous, already-shipped template | **Resolved** (this review, confirmed with precedent) |
| Is the ADR-008 task-sequencing (section 8 before section 9) correct? | Yes — ADR-008 must describe a completed, verified scope, so it belongs after integrated validation, not before. Gap found and fixed: added task 9.4 for Markdown validation of the new ADR file/README row | **Resolved** (this review) |
...
|---|---|---:|---:|
| 1 | Are all 125 skill indexes valid against an unchanged local copy of PML 0.8.0? | High | Medium-High (raised from Medium — see note) |
| 2 | Will changing schema resolution and adding validation leave generated output unchanged? | High | High |
| 3 | ~~Can the same local schema resource be resolved consistently by XML tools, runtime code, Maven tests, and CI?~~ | — | **Resolved this review** — moved to Resolved Design Questions; none of the four layers depend on the in-file attribute |
...
Note on Question 1's raised confidence: this review directly fetched and inspected the actual PML 0.8.0 `pml.xsd` (2026-07-23) rather than treating it as unknown. Findings: (a) the schema is very permissive — only `<goal>` is a required child of `<prompt>`; everything else, including `metadata`, `description`, and `title`, is optional — so most files are structurally very likely to pass; (b) the three files using the `xi:include`-in-CDATA pattern are confirmed safe against the schema's lax `xs:any` wildcards in `goal`/`step-content`. The remaining risk is narrower and more specific than originally framed: element mis-ordering against the schema's strict `xs:sequence`, or use of an undeclared element name, in any of the 125 files not yet individually checked. Task 2.4's full-inventory `xmllint` pass remains the correct, sufficient verification step — this review does not replace it, only narrows what it should watch for.
