...
- [ ] 1.5 Move `EmbeddedAgentBundleTests` from `SkillsGeneratorTest` into `agents-generator` (for example `AgentIndexesTest`) and relocate `skills-generator/src/test/resources/gherkin/agents/`; update asset classpath assertions from `skill-references/assets/agents/` to `agents/`.
- [ ] 1.6 Add a one-way Maven dependency from `skills-generator` to `agents-generator`.
- [ ] 1.7 Implement the `generate-resources` bridge in `skills-generator` using sibling-module copy into `target/generated-resources/` and expose staged files at `skill-references/assets/agents/` and `skill-references/assets/java-agents-inventory-template.md` on the main classpath.
- [ ] 1.8 Remove committed agent markdown copies from `skills-generator` once the bridge is working.
- [ ] 1.9 Add `AgentBridgeTest` and `AgentInstallerParityTest` in `skills-generator` for staged assets and `005-agents-installation.xml` XInclude parity against `AgentIndexes.agentFiles()`.
- [ ] 1.10 Add or extend skill propagation tests so generated `002` and `005` references embed staged agent content and fail when staging breaks.
