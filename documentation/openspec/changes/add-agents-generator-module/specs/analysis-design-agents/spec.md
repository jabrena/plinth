...
- **AND** active agent sources are owned by `agents-generator`, not `skills-generator`
...
- **AND** agent markdown assets that define routing are owned by `agents-generator`
...
- Derivation direction: module extraction decision -> updated agent bundle ownership -> preserved installer and inventory behavior through `skills-generator` bridge.
