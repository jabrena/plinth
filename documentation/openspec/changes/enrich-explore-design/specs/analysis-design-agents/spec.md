...
### Requirement: Architecture design exploration skill on architect mission 1

**Reason**: Skill `034-architecture-design-exploration` is retired; design refinement is owned by `/explore-design` and the enriched design skill set.

**Migration**: Remove `@034-architecture-design-exploration` from `@robot-architect` mission 1 and route design refinement through `/explore-design`.

#### Scenario: Remove skill 034 from architect routing

- **WHEN** agent source assets and generated agent prompts for `robot-architect` are inspected after the change ships
- **THEN** mission 1 does not reference `034-architecture-design-exploration`
- **AND** the design refinement step lists skills `051`–`057`, `121`–`123`, and `130` instead of skill `034`
