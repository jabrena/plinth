...
- [x] **Move Java sources and tests** from `system-prompts-generator/src/` into `plinth-skills-generator/src/` (or new unified module path), resolve package names and any duplicate classes.
...
- [x] **Fix `plinth-skills-generator`'s dependency** — Today it depends on `cursor-rules-java-generator`; after merge, that dependency becomes **in-module** (remove inter-module dependency or replace with internal packages).
...
- [x] **Regenerate skills idempotently** — Run `./mvnw clean install -pl plinth-skills-generator` so `skills/` is regenerated; then confirm **`git status` is clean** (no diffs under `skills/`). A successful merge produces deterministic output, so a second full generation should not change tracked files.
