...
Issue [#1035](https://github.com/jabrena/plinth/issues/1035) extracts command ownership from `skills-generator` into `commands-generator`. Today:
...
- Command markdown assets live under `skills-generator/src/main/resources/skill-references/assets/commands/`.
- `CommandIndexesTest` enforces parity between inventory, installer XIncludes, inventory template rows, and per-command routing contracts; it contains `//TODO Move to commands-generator ASAP`.
...
Phase 1 previously merged generators to remove cross-generator coupling. This change reintroduces a deliberate one-way dependency: `skills-generator` → `commands-generator`.

## Goals / Non-Goals

**Goals:**

- Own command inventory, assets, loader, and command-focused tests in `commands-generator`.
- Preserve command bundle behavior required by `analysis-design-commands`.
- Bridge command assets into `skills-generator` during `generate-resources`.
...
- Extract agents into `agents-generator` ([#1036](https://github.com/jabrena/plinth/issues/1036)).
...
Create `commands-generator`, relocate command sources and command-owned tests, wire the one-way dependency, and stage bridged assets into `skills-generator` without changing command contracts or generated skill semantics.
...
- Removing or renaming a command in `commands.xml` without updating generated template output fails `commands-generator` or bridge parity tests.
...
```text
commands-generator (source of truth)
...
├── depends on commands-generator (compile)
...
flowchart TB
  ...
  subgraph SG[skills-generator generate-resources]
    UNPACK[unpack commands-generator JAR]
...
Use directory and artifact id `commands-generator` with `groupId` `info.jab.pml`, matching the `skills-generator` pattern and the simpler `markdown-validator` module naming style.

Alternative considered: `cursor-rules-java-commands-generator`. Rejected as unnecessarily long for a reactor module contributors invoke with `-pl`.

### `commands-generator` is the source of truth

`commands-generator` owns `commands.xml`, `commands/*.md`, `CommandIndexes.java`, command contract tests, and command Gherkin features.
...
`skills-generator` declares a Maven dependency on `commands-generator` and, during `generate-resources`:

1. Unpacks `commands/*.md` and optional `commands.xml` from the `commands-generator` artifact into `target/generated-resources/`.
...
Implementation preference: `maven-dependency-plugin:unpack-dependencies` scoped to `info.jab.pml:commands-generator`, followed by `maven-resources-plugin:copy-resources` or `build-helper-maven-plugin:add-resource` so tests and `SkillReferenceGenerator` continue resolving `skill-references/assets/commands/*` without XML edits.
...
`skills-generator` depends on `commands-generator`. `commands-generator` does not depend on `skills-generator`.

### Move `InventoryXmlLoader` to `commands-generator`

Move `InventoryXmlLoader` into `commands-generator` with `CommandIndexes` and make it `public` so `SkillIndexes` and `SkillTriggerInventoryTest` reuse it through the module dependency.
...
|--------|--------|----------|
| Manifest integrity, asset presence, per-command contracts | `commands-generator` | `commands.xml` order, `commands/update-issue.md` routing |
| Installer XInclude parity with manifest | `skills-generator` | `004-commands-installation.xml` includes exactly manifest files |
| Bridge staging correctness | `skills-generator` | staged assets match `CommandIndexes.commandFiles()` |
| Generated skill propagation | `skills-generator` | `001` / `004` references embed staged command bodies |

The installer parity test must remain in `skills-generator` because `004-commands-installation.xml` stays there. It should call `CommandIndexes.commandFiles()` from the dependency instead of duplicating manifest parsing.

### Runtime installer path in the Plinth repo

For `004-commands-installation`, update the repository copy source path to `commands-generator/src/main/resources/commands/`. For skill-only installs from `skills/`, embedded reference content remains the fallback.

### Generated-resources must not be committed

Bridged command assets under `skills-generator` source tree are removed after the bridge works. Only `target/generated-resources/` (or equivalent) holds staged copies during build. This prevents dual ownership from returning.

## Component Boundaries

| Component | Owns | Consumes |
|-----------|------|----------|
| `commands-generator` | `commands.xml`, `commands/*.md`, loaders, command tests | parent POM only |
| `skills-generator` bridge | generated `skill-references/assets/commands/` staging | `commands-generator` JAR |
| `skills-generator` skills | `001` / `004` XML, installer wording, skill Gherkin | staged command assets |
| Agents at runtime | generated `.agents/skills` or `skills/` | not Maven modules |

## Data Flow

1. Contributor edits `commands-generator/src/main/resources/commands/*.md` or `commands.xml`.
2. `./mvnw clean verify -pl commands-generator` validates manifest, assets, and contracts.
3. `./mvnw clean install -pl skills-generator -am` rebuilds `commands-generator`, unpacks assets, stages them for XInclude, runs skill generation tests, copies output to `.agents/skills/`.
4. Skill `004` may copy from `commands-generator/.../commands/` when run inside the Plinth repo; otherwise it uses embedded reference markdown.

## Failure Handling

| Failure | Expected behavior |
|---------|-------------------|
| Missing command asset for manifest entry | `commands-generator` tests fail |
| `004` XInclude list diverges from manifest | `skills-generator` installer parity test fails |
| Bridge not run or stale JAR used | skill propagation / bridge tests fail; docs require `-am` |
| Contributor runs `-pl skills-generator` without `-am` after command edits | may use stale `~/.m2` artifact; document and optionally add reactor-order note in failure message |
| `InventoryXmlLoader` left package-private | `skills-generator` compile failure until visibility fixed |

## Testing Strategy

**Unit / focused tests (`commands-generator`)**
...
- Update `001-commands-inventory.feature` and `004-commands-installation.feature` to assert generated skill content and `commands-generator` source paths
- Keep command command-Gherkin under `commands-generator`; skill Gherkin under `skills-generator`
...
- Future `agents-generator` ([#1036](https://github.com/jabrena/plinth/issues/1036)) should mirror this pattern independently.
...
- [Risk] Overlap with `agents-generator`. → Do not move agent bundle sources in this change.

## Migration Plan

1. Create `commands-generator` module and register it in root `pom.xml`.
2. Move command inventory, loader, assets, command-owned tests, and command Gherkin.
3. Make `InventoryXmlLoader` public; update `skills-generator` imports.
...
- **Artifact id**: `commands-generator`
...
- **`InventoryXmlLoader`**: move to `commands-generator`, reuse via dependency
