...
| - | Improve a Java class method using results from a JMH analysis | Add JMH support using the cursor rule `@112-java-maven-plugins` (For Maven projects without modules). In Order to design a JMH Benchmark use the following User prompt: `Can you create a JMH benchmark in order to know what is the best implementation?` and add in the context the Java class that you want to benchmark. Once you execute the Benchmark and you have generated the JSON file, you can analyze the results with: `Can you explain the JMH results and advice about the best implementation?` Add in the context the JMH report in JSON format **Note:** | User prompt |
