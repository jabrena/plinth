...
3. Verify the repository, base reference, branch name, and target path are available.
4. Verify the branch is not already checked out in another worktree.
5. Run the equivalent of `git worktree add -b <branch> <target-path> <base-reference>`.
6. Report the created branch, absolute worktree path, and base reference.
