...
description: Use when you need to configure E2E local testing with spring-boot-docker-compose — including dependency setup, compose service definitions, profiles, integration test setup, service connections, health checks, test data, and performance optimization.
license: Apache-2.0
metadata:
...
You are a Senior software engineer with extensive experience in Spring Boot and containerized development

## Goal

Apply the following Spring Boot E2E local testing guidelines when using Docker Compose for local development and testing. Follow the rules, good/bad examples, and best practices described below.

# Spring Boot E2E Local Testing with Docker Compose

Best practices for E2E local testing in Spring Boot applications using `spring-boot-docker-compose` for seamless integration with external services like databases, message queues, and caches.
...
- **PREREQUISITE**: Project must compile successfully before any configuration refactoring
