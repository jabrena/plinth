...
- Tech leads and reviewers inspect small direct commits to `main`, architecture
  impacts, test coverage, and generated documentation as part of continuous
  delivery.
...
Software engineers use Extreme Programming with direct commits to the `main`
branch for every change. They practice pair or mob programming, test-driven
development, continuous refactoring, collective code ownership, and sustainable
pace. GenAI tools assist with design exploration, code generation, refactoring
suggestions, test creation, documentation updates, and commit summaries.
Engineers remain responsible for reviewing generated output, validating
behavior, and accepting or rejecting changes before committing to `main`.
...
The main branch pipeline validates every direct commit to `main`. It checks
formatting, compilation, unit tests, service-level integration tests, API contract
compatibility, container build reproducibility, dependency health, and security
policy compliance.

After the main branch checks pass, the pipeline creates deployable artifacts from
that validated commit. It publishes versioned Java application artifacts to
Artifactory, creates signed Docker images, stores SBOMs, and pushes deployable
images to Azure Container Registry or the selected Docker registry.
...
technical concerns without centralizing business rules. Its direct-to-`main`
delivery cycle combines human software engineering judgment, GenAI-assisted
development, CI validation, Artifactory package management, Docker image
publication, and automatic Azure deployment.
