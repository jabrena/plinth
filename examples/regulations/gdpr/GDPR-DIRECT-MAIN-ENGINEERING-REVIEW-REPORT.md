# GDPR Direct-To-Main Engineering Review Report
...
- **System, service, product, or platform:** Ecommerce CheckoutService delivery-instructions change with direct-to-`main` delivery
...
- **Technical owner:** Tech leads inspect small direct commits, but no named owner is identified
...
- **Processing type:** Customer checkout, order, delivery, payment-adjacent, notification, operational telemetry, and service-delivery processing released through a direct-to-`main` CI/CD path.
- **Data subjects:** Shoppers are directly in scope. Internal engineers, platform engineers, support, and operations users may appear in commit, audit, and operational evidence.
...
- **Primary users:** Shoppers, CheckoutService, delivery service, order service, notification service, analytics pipeline, support/operations, software engineers, tech leads, platform engineers, and CI/CD automation.
...
| 2. Data subjects | Shoppers; internal engineers, platform engineers, tech leads, support, and operations users may appear in operational evidence. | Data subject scope is not formally documented. |
...
| 6. Owner review | Platform, support, operations, and tech leads are described as roles. | Named product, technical, privacy, legal, security, compliance, risk, data-governance, vendor, and procurement approvals are missing. |
...
| 12. DPIA, privacy review, or security review | Main-branch CI, unit tests, integration tests, contract tests, SAST, dependency scanning, secret scanning, image scanning, SBOM, and provenance are described. | DPIA, privacy review, data protection review, and pre-merge privacy review are not documented. |
...
| 18. Security-of-processing controls | HTTPS, WAF, private networking, Key Vault, managed identities, scans, immutable images, SBOM/provenance, and observability are described. | Field encryption, pseudonymization, audit policy, access reviews, data masking, and direct-to-main policy controls are not fully documented. |
| 19. Privacy evidence | Data-flow and deployment diagrams exist. CI/CD and security evidence are described. The feature request documents minimization rules for Kafka. | Personal-data inventory, lawful-basis handoff, DPIA/privacy review, retention, rights, transfer, breach evidence, and pre-merge privacy evidence are missing. |
...
| 21. Current release decision | Unknown from GDPR perspective. Direct commits to `main` and automated production deployment after checks are described. | Production release should be blocked or conditioned until owner review, branch protection, and privacy evidence are complete. |
| 22. Next steps | Create personal-data inventory, minimize DTOs/events/logs, add field-level authorization, rights workflow, retention/deletion controls, transfer/vendor/DPIA evidence, privacy-safe logging, breach evidence, release-readiness approvals, pre-commit review, and main-branch protection. | Required before relying on the feature for production personal-data processing. |
...
- What branch protection, required review, privacy approval, DPIA, security review, breach-response, and release-readiness evidence exists before direct-to-`main` production rollout?
...
- **Direct-to-main governance concerns:** High. The source model commits directly to `main`, so missing pre-merge review and missing protected-main controls can let personal-data schema, Kafka, retention, logging, or authorization changes reach release automation before qualified privacy review.
- **Security-of-processing concerns:** Existing controls include HTTPS, WAF, private networking, Key Vault, managed identities, main-branch CI scans, image scans, SBOM/provenance, immutable artifacts, and rollback. Missing evidence includes field-level authorization, redaction, pseudonymization, access review, data masking, direct-to-main approval controls, and privacy tests.
...
- **Security-of-processing controls:** Preserve pipeline security controls and add field-level authorization, migration review, schema compatibility checks, non-production masking, audit trails, least-privilege service accounts, and policy-as-code gates for personal-data changes.
...
- **Testing and validation:** Add database migration tests, API validation tests, order aggregate mapping tests, Kafka schema version `3` contract tests, old-consumer compatibility tests, field-level authorization tests, privacy-safe logging tests, retention/deletion tests, rights-workflow tests, and branch-policy validation.
...
| Data-flow diagram | `expected-system-deployment.puml` shows Azure edge, AKS, data services, external providers, and delivery pipeline. | No privacy-specific data-flow annotations, data categories, transfer regions, or direct-to-main privacy gates. |
...
| Security review | WAF, private networking, Key Vault, managed identities, scans, SBOM, provenance, main-branch checks, and rollback are described. | Field-level authorization, pseudonymization, privacy-safe logging, access review, masking evidence, and branch-protection evidence are missing. |
...
| ------------- | ------ | ---------- | ---------- | ----- | ------------------- |
| Direct-to-`main` delivery can bypass privacy review | Personal-data schema, Kafka, logging, or retention changes may reach automated release before qualified review | High | Require protected `main`, code-owner review, privacy/security approval, and policy gates for personal-data changes | Platform / security / privacy | Must be remediated before production privacy approval |
...
- **Blockers:** Direct commits to `main` without documented privacy gate, missing field-level inventory, missing lawful-basis handoff, missing role documentation, missing privacy review/DPIA decision, missing retention/deletion policy, missing rights workflow, missing transfer/vendor evidence, missing privacy-safe logging evidence, missing breach-response handoff, and missing explicit note-access policy.
- **Required approvals:** Legal, privacy/DPO, compliance, security/risk, data governance, procurement/vendor owner, architecture/platform, product/business owner, technical owner, and platform owner for branch protection.
- **Expiry or review date:** Review within 30 days for direct-to-`main` governance controls, then within 90 days for privacy evidence or earlier if personal-data fields, providers, regions, analytics, AI features, logs, exports, backups, rights workflows, or checkout/delivery/payment workflows materially change.
- **Environments approved:** Development and test with synthetic or minimized data only until missing privacy evidence and branch protections are complete.
...
| -------- | ------ | ----- | -------- | ----------------- | ------ |
| High | Require protected `main`, mandatory checks, code-owner review, and privacy/security approval for personal-data schema, Kafka, logging, retention, or access-control changes. | Platform / security / privacy | 2026-06-21 | Branch protection settings, approval policy, CI evidence, and exception process | Open |
...
- **Items requiring security exception:** Any direct-to-`main` bypass for personal-data changes, logging of raw delivery notes, broad production log access, weak encryption, unmasked non-production data, unreviewed payment/authentication data handling, or missing breach evidence.
...
- **Items requiring architecture decision:** Protected-main policy, rights orchestration pattern, deletion/tombstone propagation, privacy event schema, field-level note lookup, Kafka schema compatibility, and where privacy evidence is stored.
...
- **Next review trigger:** New personal-data category, expanded note usage, new downstream consumer, direct-to-`main` exception, branch-protection change, new provider or region, production data use in non-production, analytics/export expansion, AI or RAG integration, failed deletion, breach event, or material checkout/payment/delivery workflow change.
