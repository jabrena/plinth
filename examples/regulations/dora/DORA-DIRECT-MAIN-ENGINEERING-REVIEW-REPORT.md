...
- **System, service, product, or platform:** Ecommerce deployment example, focused on the CheckoutService delivery-instructions feature request under a direct-to-main delivery model.
...
- **Technical owner:** Not identified by name; engineers and tech leads review generated output before committing to `main`.
...
- **Primary users:** Shoppers, software engineers, tech leads, platform engineers, support teams, and operations teams.
...
- **Delivery model:** Software engineers commit directly to `main`. The main branch pipeline validates every direct commit and deploys validated artifacts through development, test, staging, and production when required checks pass.
- **Diagram consistency note:** `expected-system-deployment.puml` still shows a pull-request node and short-lived branch flow, while `system-example-cicd-model.md` describes direct commits to `main`. This mismatch is itself change-control evidence that should be resolved before using the diagram as release evidence.
...
| 2. System and operational ownership | Platform engineers, support and operations teams, engineers, and tech leads are described. | Named product, technical, security, risk, resilience, and procurement owners are not identified. |
...
| 4. Material ICT assets or dependencies | Java Quarkus services, CheckoutService, PostgreSQL order database, Kafka, Key Vault, Azure platform services, main branch CI/CD, Artifactory, Docker registry, external identity provider, external payment gateway, email/SMS provider, and observability tooling. | Dependency ownership, criticality, contracts, SLAs, provider monitoring, protected-main policy, and exit paths are not described. |
...
| 7. Incident detection | Dashboards, logs, traces, alerts, Azure Monitor, App Insights, and Log Analytics are described; services emit structured logs, metrics, traces, and correlation IDs. | Specific SLOs, alert thresholds, synthetic checks, severity classification, Kafka schema compatibility alerts, direct-to-main change alerts, and business transaction monitoring are not documented. |
| 8. Escalation paths | Support and operations teams diagnose order, payment, inventory, and delivery issues. Engineers validate behavior before committing directly to `main`. | On-call, security, business, risk, resilience, provider, customer communication, post-incident review, and pre-merge review workflows are not documented. |
| 9. Incident evidence | Correlation IDs, logs, metrics, traces, alerts, dashboards, affected services, dependencies, order IDs, saga IDs, and event identifiers are described. | Incident timelines, owner approvals, communication records, mitigation records, direct-to-main approval evidence, and corrective-action evidence are not documented. |
...
| 11. Recovery targets | Rollback uses previous validated images and compatible database state. | RTO, RPO, availability targets, capacity targets, Kafka replay expectations, database restore, and manual workaround times are not defined. |
...
| 14. Change-control evidence | Direct commits to `main`, main branch checks, immutable artifacts, image signing, SBOM, provenance, deployment validation, migration safety checks, post-deployment smoke tests, and rollback decisions are described. | Missing pre-merge review, protected-main policy, named release approvals, IAM or secrets change review, provider integration review, explicit database migration approval, and Kafka schema compatibility approval. |
...
| 16. Known release blockers or gaps | The source exposes missing owner evidence, missing resilience ownership, untested restore or failover, missing provider evidence, missing formal recovery targets, and weak change-control evidence from direct-to-main delivery. | No explicit DORA release blocker list is provided. |
...
| 19. Current release decision | Unknown from a DORA perspective. The source describes production deployment after direct-to-main checks pass. | Treat as blocked for regulated production reliance until resilience owners approve the missing controls, including pre-merge review and main-branch protection. |
| 20. Next steps | Add pre-commit review or pre-merge review evidence; protect `main`; create or update ICT inventory; add monitoring, alerting, logging, and tracing evidence; add incident escalation; verify backup and restore; add continuity, failover, and rollback evidence; review third-party ICT provider controls; run resilience tests; complete release readiness evidence. | These actions should be completed before treating the service as DORA-relevant or operationally critical for a regulated financial entity. |
...
- What independent review, approval, protected-main, or separation-of-duties controls exist before direct commits become releasable production artifacts?
...
- **Critical ICT function signals:** Quarkus services, CheckoutService, PostgreSQL data stores, Kafka, Azure networking, Key Vault, Azure Monitor, Artifactory, Docker registry, main branch CI, and CD automation are material ICT assets. They are not classified as DORA critical ICT functions in the reviewed source.
...
- **Monitoring, alerting, and observability:** Convert Azure Monitor, App Insights, Log Analytics, dashboards, logs, metrics, traces, and correlation IDs into SLOs, alert thresholds, business transaction checks, dependency health checks, direct-to-main exception alerts, Kafka lag and schema-compatibility signals, database migration health checks, and evidence retention.
- **Evidence-safe logging and traceability:** Preserve order, saga, event, correlation, causation, tenant, market, deployment, migration, schema-version, commit, and approval identifiers without logging credentials, secrets, payment tokens, delivery-instruction free text, personal data, or sensitive incident details unnecessarily.
- **Incident detection and severity classification:** Define severity criteria for checkout failure, payment provider outage, Kafka lag, duplicate or missing events, incompatible event schema version `3`, database migration failure, direct-to-main release incident, identity outage, notification failure, and security monitoring gaps.
...
- **Change-control evidence:** Preserve direct-to-main commit approvals, branch-protection exceptions, release approvals, database migration review, Kafka schema compatibility approval, IAM and secrets change review, provider integration review, production validation, monitoring after change, rollback decisions, SBOM, provenance, and image-signing records.
...
| Architecture or dependency diagram | `expected-system-deployment.puml` documents Azure edge, AKS, data services, shared platform services, external systems, and a PR-based delivery pipeline. | The diagram does not match the direct-to-main system description and lacks DORA-specific dependency criticality, owner, recovery, or failure-mode annotations. |
...
| Monitoring dashboards | Azure Monitor, App Insights, Log Analytics, dashboards, metrics, traces, logs, and alerts are described. | No dashboard IDs, alert rules, SLOs, thresholds, schema compatibility alerts, direct-to-main exception alerts, or evidence-retention policy. |
...
| Incident workflow | Support and operations diagnose order, payment, inventory, and delivery issues. | No escalation, communication, incident timeline, provider handoff, post-incident review, direct-to-main exception workflow, or corrective-action workflow. |
...
| Change records | Direct-to-main commits, main branch CI checks, scans, signed artifacts, SBOM, provenance, and deployment checks are described. | Add pre-merge review, protected-main evidence, named release approvals, database migration records, Kafka contract approval, IAM/secrets review, provider change review, and production validation evidence. |
...
| Direct commits to `main` lack explicit pre-merge review | Material production changes may bypass independent review | High | Require pre-merge review or production promotion approval for material changes | Platform / engineering leadership | Block regulated production reliance until controlled |
| Protected-main controls are not documented | Unauthorized or insufficiently reviewed changes could become releasable artifacts | High | Enforce protected branch policy, required checks, restricted push rights, signed commits, and exception records | Platform / security | Requires remediation |
...
- **Blockers:** Unknown DORA applicability, missing named owners, missing pre-merge review, missing protected-main evidence, missing RTO/RPO, missing restore-test evidence, missing failover evidence, missing provider criticality records, missing SLAs/support contacts, missing incident escalation workflow, missing migration approval evidence, missing Kafka compatibility approval, missing privacy retention decision, diagram/source mismatch, and missing resilience approval.
- **Required approvals:** Legal/compliance, risk/resilience, security/privacy, procurement/provider owner, platform/SRE, architecture, database owner, event platform owner, engineering leadership, and business owner.
- **Expiry or review date:** Review within 90 days or earlier if a financial entity adopts the system, payment or settlement responsibilities expand, criticality changes, a material provider changes, Kafka contracts change, database state changes, branch policy changes, direct-to-main exceptions occur, or production incident evidence reveals a resilience gap.
- **Environments approved:** Development and test for engineering validation. Staging and production promotion require documented review and branch-protection controls if the service is business-critical or DORA-relevant.
...
| High | Replace direct-to-main production promotion with mandatory review or document an equivalent independent approval gate. | Platform / engineering leadership | 2026-06-28 | Branch policy, approval workflow, or release gate evidence | Open |
| High | Enforce protected-main controls for required checks, restricted push rights, signed commits, reviewer approval for high-risk changes, and exception records. | Platform / security | 2026-06-28 | Protected branch settings and audit evidence | Open |
...
| Medium | Add resilience release gate checks for ICT inventory, incident runbook, restore test, provider review, rollback plan, database migration approval, Kafka compatibility, protected-main evidence, and residual-risk approval. | Platform / architecture | 2026-07-26 | CI/CD or release checklist evidence | Open |
...
- **Items requiring security exception:** Any missing branch protection, secrets-management, logging, incident evidence, provider monitoring, identity, access-control, privacy, or free-text handling review.
...
- **Items requiring architecture decision:** Whether to add DORA-specific release gates, how to document direct-to-main controls, how to document ICT inventory and provider dependencies, how to represent resilience evidence in diagrams, and how to approve database and Kafka contract changes.
- **Items requiring product or business acceptance:** Residual risk from checkout, payment, order, inventory, delivery, notification, analytics outages, delayed recovery, direct-to-main change control, or delivery-instruction data handling.
- **Next review trigger:** Financial-entity adoption, EU regulated-service classification, material provider change, payment workflow expansion, Kafka schema change, database migration, branch policy change, direct-to-main exception, new production environment, failed recovery test, major incident, or missing owner escalation.
