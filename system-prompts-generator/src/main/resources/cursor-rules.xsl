...
## Constraints
    ...
    <!-- Adds blank line before ## headings to satisfy markdown formatting requirements -->
    <xsl:template name="remove-goal-indentation">
        ...
        <xsl:choose>
            <xsl:when test="contains($text, '&#10;')">
                <xsl:variable name="line" select="substring-before($text, '&#10;')"/>
                <xsl:variable name="rest" select="substring-after($text, '&#10;')"/>
                <xsl:variable name="trimmed-line">
                    <xsl:call-template name="ltrim">
                        <xsl:with-param name="string" select="$line"/>
                    </xsl:call-template>
                </xsl:variable>
                <xsl:if test="starts-with($trimmed-line, '## ') and string-length($trimmed-line) &gt; 0">
                    <xsl:text>&#10;</xsl:text>
                </xsl:if>
                <xsl:value-of select="$trimmed-line"/>
                <xsl:text>&#10;</xsl:text>
                <xsl:call-template name="remove-goal-indentation">
                    <xsl:with-param name="text" select="$rest"/>
                </xsl:call-template>
