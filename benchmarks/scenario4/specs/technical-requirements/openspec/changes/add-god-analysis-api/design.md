...
- Create Spring Boot 4.1.0 module skeleton (`info.jab.ms`), properties, `RestClient` bean, and Maven **`--enable-preview`** for **`StructuredTaskScope`**.
...
- Add validation, **`StructuredTaskScope`** parallel fetch, partial aggregation, acceptance tests.
