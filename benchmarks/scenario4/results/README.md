...
Each completed Case 4 run MUST write one JSON file under this directory:
...
| Field | How to measure |
| --- | --- |
| `acceptance_pass` | Required boolean. `true` only if the product acceptance in the functional requirements happy path is met (for Case 4: implement in `demo/` using co-located FR + OpenSpec technical requirements and satisfy documented sum behavior / operator checklist). Also require this feature’s setup constraints to hold. |
...
| Field | How to measure |
| --- | --- |
| `scenario` | Constant: `"scenario4"` |
| `case_id` | Constant: `"case-4-current-openspec-problem1"` |
...
1. Confirm Case 4 setup (full `problem1/` FR inventory plus co-located technical OpenSpec with FR-relative Source/Derivation links) before starting.
...
See also: [scenario4.feature](../gherkin/scenario4.feature), [../README.md](../README.md) metrics scorecard.
