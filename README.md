...
All XML files use one Single XML Schema, [pml.xsd](./generator/src/main/resources/pml.xsd) and those XML documents are Transformed into Markdown with Frontmatter using a [Java class](./generator/src/main/java/info/jab/pml/CursorRulesGenerator.java) and a [XSL file](./generator/src/main/resources/cursor-rules.xsl) specialized in Cursor rule format.

Review the XML documents and the process:

```bash
cd generator
./mvnw clean verify # Pass tests
./mvnw clean install # Pass teas & copy new .mdc files in ./cursor/rules (The way to promote changes)
```

When you feel confident with the process, try to create new XML documents, Models will help you because a XML file is more rigid that natural lenguage and it has `a common vocabulary` to create prompts.

When you feel confident with the solution, send a PR from your forked repository.
