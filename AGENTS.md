...
Numeric prefixes group related prompts. **Spring Boot** uses `301`–`302` (core, REST), `311`–`312` (JDBC, Spring Data JDBC), and `321`–`323` (testing). **Quarkus** uses the same shape starting at **`401`**: `401`–`402` (core, REST), `411`–`412` (JDBC, Panache), `421`–`423` (testing). **Micronaut** uses **`501`**: `501`–`502` (core, REST), `512` (Micronaut Data — relational repositories; no separate `511` JDBC prompt in this index), and `521`–`523` (testing). New Quarkus framework prompts and skills must keep the **`401-`…`423-`** range; new Micronaut prompts must keep **`501-`…`502-`**, **`512-`**, and **`521-`…`523-`** (do not reuse Spring or Quarkus prefixes).
